...
import ResearchAssistantResults from '@/components/ResearchAssistantResults'
...
export default function ResearchAssistantResultsPage() {
  ...
  useEffect(() => {
    ...
    try {
      const savedAnalyses = JSON.parse(localStorage.getItem('savedResearchAssistantAnalyses') || '[]')
      const found = savedAnalyses.find((a: SavedAnalysis) => a.id === id)

      if (found && found.cachedResult) {
        setAnalysis(found)
      } else {
        // Also check recent searches (legacy key for backward compatibility)
        ...
        if (recentFound && recentFound.cachedResult) {
          ...
          // Also check legacy AI Editor analyses
          const legacyAnalyses = JSON.parse(localStorage.getItem('savedAIEditorAnalyses') || '[]')
          const legacyFound = legacyAnalyses.find((a: SavedAnalysis) => a.id === id)

          if (legacyFound && legacyFound.cachedResult) {
            setAnalysis(legacyFound)
          } else {
            setNotFound(true)
          }
        }
